...
func main() {
	if err := execute(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "backuply:", err)
		os.Exit(1)
	}
}
...
func execute(args []string) error {
	...
	if args[0] == "version" {
		fmt.Println("backuply", version, "(single-writer-v1)")
		return nil
	}
	command := args[0]
	switch command {
	case "run", "init", "id", "check", "status":
	default:
	...
	if command == "init" || command == "id" {
		flags.StringVar(&stateDir, "state-dir", "", "identity directory (without loading a configuration)")
	}
	...
	if stateDir != "" {
		...
		if configExplicit {
			return errors.New("choose either -config or -state-dir")
		}
	...
	case "check":
		...
		return nil
	case "init":
		lock, err := daemon.LockState(cfg.StateDir)
		if err != nil {
			return err
		}
		defer lock.Close()
		...
		for _, folder := range cfg.Folders {
			if err := store.InitFolder(folder.Path, folder.ID); err != nil {
				return fmt.Errorf("initialize folder %s: %w", folder.ID, err)
			}
		}
		fmt.Println(id.ID)
		return nil
	case "id":
		...
		return nil
	case "status":
		status, err := daemon.QueryStatus(context.Background(), cfg.StateDir)
		if err != nil {
			return err
		}
		if !status.Running {
...
func usage() {
	fmt.Fprintln(os.Stdout, `Usage: backuply COMMAND [options]
