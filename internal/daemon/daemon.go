...
func New(cfg config.Config, logger *slog.Logger) (*Daemon, error) {
	...
	if err != nil {
		lock.Close()
		return nil, fmt.Errorf("load identity (run backuply init first): %w", err)
	}
	...
	for id, f := range cfg.Folders {
		d.status.Folders[id] = FolderStatus{Source: f.Source}
	}
	return d, nil
}
...
func (d *Daemon) Status() Status {
	...
	for id, f := range d.status.Folders {
		s.Folders[id] = f
	}
	return s
}
...
func (d *Daemon) update(id string, count int, stats model.Stats, err error) {
	...
	if err != nil {
		...
		s.BytesReused += stats.BytesReused
	}
	d.status.Folders[id] = s
	d.mu.Unlock()
	if err != nil && previousError != err.Error() {
		d.log.Warn("folder paused; will retry", "folder", id, "error", err)
	}
	if err == nil && previousError != "" {
		d.log.Info("folder recovered", "folder", id)
	}
	if err == nil && stats.Files > 0 {
		d.log.Info("folder synchronized", "folder", id, "files", stats.Files, "received", stats.BytesReceived, "reused", stats.BytesReused)
	}
}
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	if info, err := os.Lstat(socketPath); err == nil {
		if info.Mode()&os.ModeSocket == 0 {
			return fmt.Errorf("control path is not a socket: %s", socketPath)
		}
	...
	for name, p := range d.cfg.Peers {
		...
		for _, id := range sortedFolders(d.cfg.Folders) {
			if f := d.cfg.Folders[id]; f.Source == name {
	...
	<-results
	if errors.Is(runErr, net.ErrClosed) || errors.Is(runErr, http.ErrServerClosed) {
		return nil
	}
	return runErr
}
...
func (d *Daemon) replicate(ctx context.Context, peer config.Peer, folders []config.Folder) {
	...
	for {
		...
		for _, folder := range folders {
			if err := store.CheckFolder(folder.Path, folder.ID); err != nil {
				d.update(folder.ID, 0, model.Stats{}, err)
				continue
			}
			if client == nil {
				var err error
			...
			if err == nil {
				...
				for _, entry := range entries {
					...
					total.BytesReused += stats.BytesReused
				}
			}
...
func (d *Daemon) authorize(peerID, folderID string) (config.Folder, error) {
	...
	if !ok {
		return config.Folder{}, errors.New("folder access denied")
	}
	allowed := false
	for _, name := range f.Peers {
		if d.cfg.Peers[name].DeviceID == peerID {
			allowed = true
			break
		}
	}
	...
	if err := store.CheckFolder(f.Path, f.ID); err != nil {
		return config.Folder{}, err
	}
	if f.Source == "local" {
		d.mu.RLock()
		status := d.status.Folders[folderID]
		d.mu.RUnlock()
		if status.LastSuccess.IsZero() || status.LastError != "" {
			return config.Folder{}, errors.New("source scan is not available")
		}
	}
	return f, nil
}
...
func (d *Daemon) List(ctx context.Context, peerID, folderID string) ([]model.Entry, error) {
	if _, err := d.authorize(peerID, folderID); err != nil {
		return nil, err
	}
...
func QueryStatus(ctx context.Context, stateDir string) (Status, error) {
	...
	if err != nil {
		return Status{}, fmt.Errorf("daemon unavailable: %w", err)
	}
